...
function resizeCanvas() {
    ...
    // Set initial ball position only once
    if (!ball.initialized) {
        ball.x = canvas.width / 2;
        ball.y = canvas.height / 2 - hexagonRadius + ballRadius;
...
function updateBall() {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        const x =
            canvas.width / 2 +
            hexagonRadius * Math.cos(angle);

        const y =
            canvas.height / 2 +
            hexagonRadius * Math.sin(angle);
